"""
member2_video_fusion/models/video_model.py
==========================================
MedOracle — Member 2 (Vanaiyan)

VideoEmotionModel — the complete video branch of the MedOracle pipeline.

Wires together:
    ResNet50Encoder  →  per-frame feature extraction  (2048-dim per frame)
    EmotionBiLSTM    →  temporal modelling + classification (5-class)

Full data flow
--------------
    Input  : (B, T, 3, 224, 224)   — batch of video clips, T=16 frames each
               ↓
    Reshape: (B×T, 3, 224, 224)    — treat each frame independently
               ↓
    ResNet50: (B×T, 2048)          — per-frame feature extraction
               ↓
    Reshape: (B, T, 2048)          — restore temporal structure
               ↓
    BiLSTM : (B, 5)                — logits + probabilities
               ↓
    Output : dict  { logits, probs, predicted_emotion, confidence,
                     class_probabilities, frame_features }

This output dict feeds directly into the gated fusion module (fusion.py).

Device support
--------------
    MPS  (Apple Silicon) — primary for local training on your MacBook
    CUDA (NVIDIA GPU)    — if available on a remote server
    CPU                  — fallback
