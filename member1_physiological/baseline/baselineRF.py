...
def run_rf_loso(subject_data: dict) -> None:
    ...
    print("── Random Forest Baseline — 5-class LOSO ────────────────────────")
    ...
    for fold_idx, test_subj in enumerate(subject_ids):
        ...
        # Random Forest does not require scaling but we apply it for consistency
    ...
    print(f"\n{'═'*52}")
    print(f"Random Forest  — Mean Accuracy : {mean_acc*100:.2f}%")
    print(f"Random Forest  — Mean Macro-F1 : {mean_f1:.4f}")
